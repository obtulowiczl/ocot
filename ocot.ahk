...
Menu, Tray, Add, Poka¿ OCOT, restore
Menu, Tray, Default, Poka¿ OCOT
Menu, Tray, Click, 1
Menu, Tray, Tip, Przypominacz OCOT - OC/Przegl¹dy pojazdów

Gui, Main: New, , Przypominacz OCOT
Gui, Add, StatusBar,,
Gui, Font, s10
Gui, Add, ListView, w580 h380 vReminderListView g_ReminderList, Samochód|Rejestracja|OC wazne do:|BT wa¿ne do:
LV_ModifyCol(1, "+center 265")
LV_ModifyCol(2, "+center 90")
LV_ModifyCol(3, "+center 110")
LV_ModifyCol(4, "+center 110")
Gui, Font, Normal
Gui, Add, Button, h25 w140 x5 y400 vButtonDelete g_DeleteRow, Usuñ wybrany pojazd
Gui, Add, Button, h25 w140 x+7 yp vButtonAdd g_AddCar, Dodaj nowy pojazd
Gui, Font, Bold s10
Gui, Add, Button, h25 w225 x+7 yp vButtonUpcoming g_GetUpcoming, Poka¿ nadchodz¹ce terminy
Gui, Font, Normal s7
...
{
	TrayTip,UWAGA!,Wkrótce trzeba przed³u¿yæ OC/BT. `nSzczegó³y w komunikacie., 5, 2
	_GetUpcoming()
}
}
...
Gui, Font, Normal
Gui, Add, Text, x10 yp+40, Polisa OC wa¿na do:
Gui, Font, Bold
...
Gui, Font, Normal
Gui, Add, Text, x10 yp+35, Badanie techniczne wa¿ne do:
Gui, Font, Bold
...
Gui, OCOTSetting: Add, Text, x10 y10 ,Informuj o koñcz¹cych siê OC:
Gui, OCOTSetting: Add, Edit, yp-2 x+5 w35 +center vSettingsOC , %SetDniOC%
Gui, OCOTSetting: Add, Text, yp+2 x+2 , dni wczeœniej.

Gui, OCOTSetting: Add, Text, x10 y+20 ,Informuj o koñcz¹cych siê BT:
...
Gui, OCOTSetting: Add, Button, x10 yp+30 w330 g_Exit,ZAKOÑCZ PROGRAM (wy³¹cza powiadomienia!)
...
Loop
{
    ...
    if not RowNumber  ; The above returned zero, so there are no more selected rows.
        break
	MsgBox, 292, PotwierdŸ usuniêcie, Czy na pewno chcesz usun¹æ zaznaczony pojazd?
		IfMsgBox, Yes
			LV_Delete(RowNumber)
}
_updateDB()
...
Gui, Upcoming: Add, Text, , Wygasaj¹ce polisy OC w najbli¿yszym czasie (%SetDniOC% dni):

LV_ModifyCol(3, "Sort")
listOC =
listBT = Badanie techniczne koñczy siê w:
m := 1
...
Gui, Upcoming: Font, s10 bold
Gui, Upcoming: Add, Text, x10 yp+45, Koñcz¹ce siê badania techniczne (najbli¿sze %SetDniBT% dni):
...
Gui, Upcoming: Show, AutoSize, Wygasaj¹ce polisy / koñcz¹ce siê badania:
_sortList()
}
...
if (LV_GetCount() > 0)
{
	...
	EnvSub, incoming3, %czas%, days
		SB_SetText("Najbli¿sza p³atnoœæ OC za " incoming3 " dni ("incomingOC1 A_Space incomingOC2 ")", 1)
}
...
if (LV_GetCount() > 0)
{
	...
	SB_SetText("Najbli¿sze BT za " incoming4 " dni ("incomingBT1 A_Space incomingBT2 ")", 2)
}
